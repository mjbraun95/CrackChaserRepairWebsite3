<?php $title = 'Crack Chaser Repair Solutions'; ?>
<?php $currentPage = 'Home'; ?>
<?php include('head.php'); ?>
<?php include('navbar.php'); ?>

<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>
    /* Your existing styles */
    .responsive {
        max-width: 100%;
        height: auto;
        object-fit: scale-down;
        margin: auto;
    }

    /* Additional styles for the background image */
    #SLIDE_BG {
        background-image: url('https://www.pavementgroup.com/images/products/uep-1357.jpg');
        background-size: contain; /* Scale the image to the largest size such that both its width and its height can fit inside the content area */
        background-position: top center; /* Position the image at the top center of the container */
        background-repeat: no-repeat;
        color: #ffffff; /* White text for contrast */
        text-align: center; /* Center align the text */
        padding: 50px 0; /* Adjust padding as needed */
        max-width: 800px; /* Maximum width of the image's resolution, adjust as needed */
        margin: auto; /* Center the container */
    }

    /* Styles for the text inside the content area */
    .content {
        width: 80%;
        margin: auto; /* Centers the content */
    }

    .fullBox {
        margin-bottom: 20px; /* Adds space between paragraphs */
    }

    .fullBoxBold {
        font-weight: bold;
        font-size: 1.5em;
        margin-bottom: 30px; /* Adds a little more space under the bold paragraph */
    }
</style>

<!-- Background image URL set in the style tag above -->
<div id="SLIDE_BG">
    <div class="content">
        <p class="fullBoxBold">   
            Professional Asphalt, Concrete, and Epoxy Floor Services
        </p>
        <p class="fullBox">
            Looking for a durable solution for your damaged pavement, flooring, or concrete structures at a reasonable price? Look no further. 
        </p>
        <p class="fullBox">
            Crack Chaser Repair Solutions is an Edmonton based repair & restoration company, proudly serving Alberta. We bring over 20 years of experience with cost-efficient solutions to your asphalt, concrete, and epoxy floor needs. 
        </p>
        <p class="fullBox">
            We work on industrial, residential, and commercial properties. 
        </p>
        <p class="fullBox">
            Our team utilizes modern methods and cutting-edge technologies to ensure you get long-lasting, quality repairs. 
        </p>
    </div>
</div>
</body>
</html>
